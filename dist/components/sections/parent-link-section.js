 ...
 */
export function ParentLinkSection({ pb }) {
    ...
    const loadConfig = useCallback(async () => {
        try {
            const res = await pb.send("/api/parentlink/config", { method: "GET" });
            setConfig(res);
    ...
    const saveConfig = async () => {
        setSaving(true);
        try {
            const res = await pb.send("/api/parentlink/config", {
                method: "POST",
                body: {
